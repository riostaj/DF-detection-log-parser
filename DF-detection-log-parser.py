            ...
            if current_event:
                ...
                if add_match:
                    ...
                    if add_match.group("stop"):
                        ...
                        # Calculate Stop Time if missing
                        try:
                            start_dt = datetime.strptime(current_event["Start Time (UTC)"], "%Y-%m-%d %H:%M:%S")
                            duration_sec = int(current_event["Duration (sec)"])
                            stop_dt = start_dt + timedelta(seconds=duration_sec)
                            current_event["Stop Time (UTC)"] = stop_dt.strftime("%Y-%m-%d %H:%M:%S")
                        except Exception as e:
                            print(f"Error calculating Stop Time: {e}")
